...
class AdminVaccineViewSet(viewsets.ModelViewSet):
    """
    Admin-only endpoints to create vaccines and verify vaccination records
    """
...
class AdminUserVaccineRecordViewSet(viewsets.GenericViewSet):
    """
    Admin-only actions for vaccination records
    """
    queryset = UserVaccineRecord.objects.all()
    ...
    def verify(self, request, pk=None):
        """
        (2) Verify an unverified vaccination record
        """
        try:
            record = self.get_object()
        except UserVaccineRecord.DoesNotExist:
        ...
        record.save()
        return Response({"detail": f"Vaccination record {record.id} verified successfully."})
